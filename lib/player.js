...
class DemoPlayer {
	constructor(demo, icons) {
		...
		this.inspector_button = document.createElement("input");
		this.inspector_button.id = "inspector_checkbox";
		this.inspector_button.type = "checkbox";
		this.inspector_button.addEventListener("click", () => {this.should_update_inspector = this.inspector_button.checked;});
		controls_section.appendChild(this.inspector_button);

		this.inspector_label = document.createElement("label");
		this.inspector_label.innerHTML = "Show contents";
		this.inspector_label.setAttribute("for","inspector_checkbox");
		controls_section.appendChild(this.inspector_label);
		...
		this.should_update_inspector = false;
	...
	fix_chat_message(message) {
		...
		for (var i = 0; i < message.length; i++) {
			...
			if(code == 182) {
				message2 += String.fromCharCode(1103);
				continue;
			}
	...
	add_chat(chat, index) {
		...
		div.innerHTML = this.fix_chat_message(chat.message);
	...
	frame(timestamp) {
		...
		for(let type_array of this.object_ref_list) {
			...
			if(this.should_update_inspector)
